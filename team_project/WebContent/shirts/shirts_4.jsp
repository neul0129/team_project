<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>

</head>
<body>
	<div class="headingArea ">
		<div class="detailArea ">
			<div class="imgArea ">
				<div class="listImg">
					<ul>
						<li class="xans-record-"><img
							src="//www.margarethowell.co.kr/web/product/small/202102/b8386d8aa51f77c66d90bd4f24d4ed58.jpg"
							class="ThumbImage" alt=""></li>
						<li class="xans-record-"><img
							src="//www.margarethowell.co.kr/web/product/extra/small/202102/199f5ce1ef36758f082e625cc2ce89b1.jpg"
							class="ThumbImage" alt=""></li>
						<li class="xans-record-"><img
							src="//www.margarethowell.co.kr/web/product/extra/small/202102/496bbe28785edd6cd83607d39b2dd299.jpg"
							class="ThumbImage" alt=""></li>
					</ul>
				</div>
			</div>


			<div class="infoArea">
				<div class="product-detaildesign">
					<tr class=" xans-record-">
						<td><div style="font-size: 14px; color: #6a696c;">
								MHL.<br> DRY COTTON SLUB <br> BEIGE
							</div></td>
					</tr>
					<tr class=" xans-record-">
						<td style="line-height: 18px !important;"><span
							style="font-size: 11px; color: #6a696c;">코튼 소재의 스프레드 칼라
								셔츠입니다. <br> 여성용 제품입니다. <br> <br> 원산지 : 일본 <br>
								품번 : MHP42M0001BE <br> 혼용율 : 면 100%<br> 세탁방법 <br>
								30℃에서 중성세제로 약하게 세탁하십시오.<br> 염소, 산소 표백은 피해주십시오. <br>
								사이즈 가이드 <br>1 : 가슴둘레 102cm, 총길이 55cm, 소매길이 73.5cm <br>2
								: 가슴둘레 107cm, 총길이 60cm, 소매길이 76.5cm <br>
						</span></td>
					</tr>
					<tr class=" xans-record-">
						<td style="line-height: 18px !important;"><div
								style="margin: 10px; font-size: 14px; color: #6a696c;">
								<strong id="span_product_price_text">428,000</strong><input
									id="product_price" name="product_price" value="" type="hidden">
							</div></td>
					</tr>
				</div>

				<tbody class="option_button">
					<tr>
						<th scope="row">SIZE</th>
						<div class="button_type">
							<button id="product_btn1" type="button">1</button>
							<button id="product_btn2" type="button">2</button>
						</div>
					</tr>
				</tbody>

				<div class="displaynone1">
					<label>1:</label>
					<p>MHL. DRY COTTON SLUB</p>
					<span>428,000</span>
				</div>

				<div class="displaynone2">
					<label>2:</label>
					<p>MHL. DRY COTTON SLUB</p>
					<span>428,000</span>
				</div>


				<div class="option_button2 ">
					<div class="buttonArea">
						<span class="gLeft"> <a href="#"><button type="button"
									id="button_addtobag">
									<span style="font-size: 14px;">ADD TO BAG</span>
								</button> </a> <a href="#"><button type="button" class="review"
									style="margin-top: 10px;">REVIEW</button></a>
						</span>
					</div>
				</div>
			</div>
		</div>
	</div>
</body>
</html>